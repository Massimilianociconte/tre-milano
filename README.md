...
- Pagina `/esplora/` con filtri per testo, categoria e quartiere, badge di verifica e paginazione; 4.142 sedi bronze mappate sulle 22 zone editoriali via alias NIL; workflow di revisione umana bronze→silver (`scripts/data-review-venue.mjs`) tracciato in `venue_update_history`; cache in-memory delle interpretazioni DeepSeek (stessa query, una sola chiamata) senza persistenza del testo.
